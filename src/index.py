...
class Util:  # 统一的类
    ...
    """
    获取时间方法，并将时间以 "%Y-%m-%d %H:%M:%S" 形式返回
    """

    @staticmethod
    def getTimeStr():
        # 获取系统的“世界标准时间”，因为服务器使用的时间是utc时间
        # 拿到UTC时间，并强制设置时区为UTC+0:00:
        utc_time = datetime.utcnow().replace(tzinfo=timezone.utc)
        # astimezone 方法是将转换时区为北京时间:
    ...
    Log日志打印函数
        content：打印的内容
        show:是否使用，默认使用
        sys.stdout.flush()：当我们打印一些字符时，并不是调用print函数后就立即打印的。一般会先将字符送到缓冲区，然后再打印。
    ...
    @staticmethod
    def GetLoginParams(html_src):
        result = {}
        form = re.findall(r'<form.*?>(.*?)</form>', html_src, re.DOTALL)
        if len(form) < 1:
            return None
        form = form[0]
    ...
    """
    百度OCR识别（https://ai.baidu.com/tech/ocr/general）
    """
    ...
    def SendMessage(title: str, content: str, topic='', ctype='html'):
        if PUSHPLUS_token == '':
            Util.log("未配置pushplus的token，消息将不会推送")
            return False
        data = {
...
class AutoSign:
    ...
    def GetTasks(session, apis):
        res = session.post(url=API['Sign']['GETTasks'].format(host=apis['host']), headers=Util.GenNormalHears(),
                           data=json.dumps({}))
        res = res.json()
    ...
    def GetDetailTask(session, params, apis):
        res = session.post(url=API['Sign']['GETDetail'].format(host=apis['host']), headers=Util.GenNormalHears(),
    ...
    def submitForm(session, user, form, apis):
        res = session.post(url=API['Sign']['Submit'].format(host=apis['host']),
                           headers=Util.GenHeadersWithExtension(user, apis), data=json.dumps(form))
    ...
    def GenInfo(username, password, apis):
        ...
        res = session.post(
            url='https://{}/wec-counselor-sign-apps/stu/sign/getStuSignInfosByWeekMonth'.format(apis['host']),
